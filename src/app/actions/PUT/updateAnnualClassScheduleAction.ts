    ...
    try {
        ...
        const result = await executeQuery(async () => {
            ...
            if (teacherIds.length > 0 && subjectIds.length > 0) {
                const pairs = createTeacherSubjectPairs(teacherIds, subjectIds);
        ...
        dbLog({ description: `Error updating annual class schedule entry: ${error instanceof Error ? error.message : String(error)}`, schoolId });
        return {
            success: false,
