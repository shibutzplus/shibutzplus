...
import { getSessionMaxAge, TWENTY_FOUR_HOURS } from "@/utils/time"; // removed mathFloorNow
...
// If we used a fixed value from server start, new users could get an already expired session.
  ...
  callbacks: {
    ...
    async jwt({ token, user, account, profile }) {
      if ((account?.provider === "google" && profile?.email) || (user && user.email)) {
        ...
        token.exp = nowInSec() + Number(token.maxAge); // set exp from current time, not module-load time
      }
      if (token.maxAge && (!token.exp || Number(token.exp) < nowInSec())) {
        token.exp = nowInSec() + Number(token.maxAge); // refresh exp if missing/expired
      }
      return token;
    },
