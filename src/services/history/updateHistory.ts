...
export async function processHistoryUpdate(dateString?: string): Promise<HistoryUpdateResult> {
    ...
    try {
        ...
        if (schedules.length > 0) {
            ...
            historyRecords = schedules.map(schedule => {
        ...
        // 5. Run DB operations in a batch for atomicity (neon-http doesn't support interactive transactions)

        // Prepare cleanup date
        ...
        // Prepare the queries
        const deleteOldHistoryQuery = db.delete(history)
            .where(
                and(
        ...
        const cleanupScheduleQuery = db.delete(dailySchedule)
            .where(lte(dailySchedule.date, cleanupDateStr));

        const batchQueries: any[] = [deleteOldHistoryQuery];

        if (historyRecords.length > 0) {
            batchQueries.push(db.insert(history).values(historyRecords));
        }

        batchQueries.push(cleanupScheduleQuery);
        ...
        try {
            ...
            // Invalidate the entire history path as a fallback
            revalidatePath('/history', 'page');
            revalidatePath('/(private)/history', 'page');
            ...
            // next/cache might not be available in all environments
        }
