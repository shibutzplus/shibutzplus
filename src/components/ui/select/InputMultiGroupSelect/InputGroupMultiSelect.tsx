...
export interface InputGroupMultiSelectProps {
    ...
    /** Selected values */
    value?: string[];
    /** Called with selected values (value[]) */
    onChange: (value: string[], removedValue?: string) => void;
    placeholder?: string;
...
const InputGroupMultiSelect: React.FC<InputGroupMultiSelectProps> = ({
    ...
    // Keep local grouped options in sync with incoming prop
    useEffect(() => {
        setGroupedOptions(groupedInitial);
    }, [groupedInitial]);

    // Map incoming value[] to actual option objects
    useEffect(() => {
        if (!value || value.length === 0) {
            setSelectedOptions([]);
            return;
        }
        const all = groupedOptions.flatMap((g) => g.options);
        const mapped = value
