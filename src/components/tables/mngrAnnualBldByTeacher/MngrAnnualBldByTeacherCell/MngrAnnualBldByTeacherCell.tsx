...
import styles from "./MngrAnnualBldByTeacherCell.module.css";
...
type MngrAnnualBldByTeacherCellProps = {
    day: string;
    hour: number;
...
const MngrAnnualBldByTeacherCell: React.FC<MngrAnnualBldByTeacherCellProps> = ({
    day,
    hour,
    schedule,
    selectedTeacherId,
    ...
    const confirmRemove = (what: string | null, proceed: () => void) => {
        const cellData = schedule[selectedTeacherId]?.[day]?.[hour];
        const isFull = (cellData?.subjects?.length ?? 0) > 0 && (cellData?.classes?.length ?? 0) > 0;
...
export default MngrAnnualBldByTeacherCell;
